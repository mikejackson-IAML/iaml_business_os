...
## Objective
Create the main page.tsx server component with Suspense pattern and a loading skeleton.

## Context
Following the established pattern from leads/page.tsx and programs/page.tsx:
- Server component exports metadata and revalidate
- Async data loader function
- Suspense wrapper with skeleton fallback
- Data passed to content component

## Tasks

<task id="1">
Create directory structure for the faculty-scheduler dashboard:
```bash
mkdir -p dashboard/src/app/dashboard/faculty-scheduler/components
```
</task>

<task id="2">
Create file `dashboard/src/app/dashboard/faculty-scheduler/page.tsx`:
...
<task id="3">
Create file `dashboard/src/app/dashboard/faculty-scheduler/faculty-scheduler-skeleton.tsx`:
...
## Verification
- [ ] Directory created: dashboard/src/app/dashboard/faculty-scheduler/
- [ ] page.tsx created with Suspense pattern
- [ ] Exports metadata with title and description
- [ ] Exports revalidate = 300 for 5-minute caching
- [ ] Uses async data loader function pattern
- [ ] skeleton.tsx created with matching layout structure
- [ ] Skeleton uses existing dashboard-kit components (Skeleton, Card)
- [ ] Layout matches planned content: header, 6-col summary cards, 12-col grid (8+4)

## must_haves
- Page follows established Next.js 14 patterns
- Metadata configured for SEO
- 5-minute revalidation for fresh data
- Skeleton matches final layout for smooth loading transition
