...
depends_on:
  - 05-05-PLAN.md
files_modified:
  - dashboard/src/app/dashboard/faculty-scheduler/components/recruitment-pipeline-table.tsx
autonomous: true
---

# 05-06: Recruitment Pipeline Table

## Objective
Create the main table component showing all programs with tier status, days remaining, notification stats, and action buttons (B2, B3, B4, B5, B6, B8).

## Context
Requirements implemented:
- B2: Status per program (Awaiting Tier 0 → Tier 1 → Open → Claimed → Confirmed)
- B3: All required columns (name, date, location, tier, days left, notified, responded, assigned, actions)
- B4: Action: Manually assign instructor
- B5: Action: Skip tier early
- B6: Action: Send reminder nudge
- B8: Action: Override/remove existing claim

## Tasks

<task id="1">
Create file `dashboard/src/app/dashboard/faculty-scheduler/components/recruitment-pipeline-table.tsx`:
    ...
    <tr className="border-b border-border/50 hover:bg-background-card-light/50">
      ...
      <td className="py-3 text-right">
        <div className="relative">
          ...
          {showActions && (
            <div className="absolute right-0 top-full mt-1 w-48 rounded-md border border-border bg-background-card shadow-lg z-10">
              <div className="p-1">
                {canSkipToTier1 && (
                  <button
                    className="flex w-full items-center gap-2 rounded px-3 py-2 text-sm hover:bg-background-card-light"
                ...
                {hasClaim && (
                  <button
                    ...
                    onClick={() => {
                      // TODO: Need to get claim ID - for now pass program ID
                      // The override modal will need to fetch claims for this program
...
<task id="2">
Add click-outside handler to close the actions dropdown when clicking elsewhere on the page. This can be added using useEffect with a document click listener, or by adding a transparent overlay behind the dropdown.
</task>

## Verification
- [ ] Table displays all required columns (B3)
- [ ] Status badges show tier status with distinct colors (B2)
- [ ] Days remaining shows warning color when < 2 days
- [ ] Notified and Responded columns show counts
- [ ] Assigned column shows instructor name when filled
- [ ] Action dropdown shows Skip Tier options (B5)
- [ ] Action dropdown shows Send Reminder (B6)
- [ ] Action dropdown shows Assign Instructor (B4)
- [ ] Action dropdown shows Override Claim when applicable (B8)
- [ ] Actions call server actions with loading state
- [ ] Empty state shown when no programs

## must_haves
- All B2-B6, B8 requirements implemented in table
- Status badges clearly differentiate tier stages
- Action buttons correctly enabled/disabled per program state
- Pending state disables buttons during action execution
