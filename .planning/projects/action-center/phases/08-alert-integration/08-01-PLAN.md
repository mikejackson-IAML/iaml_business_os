...
# Plan 08-01: Alert Webhook Schema and Standardization

## Objective

Create a standardized alert structure that all Business OS components can use when sending alerts to the Action Center. This establishes the webhook payload format and alert metadata tables.

## Context

From 08-CONTEXT.md:
- Alerts come from multiple sources: SSL monitors, domain health, uptime, payments, faculty scheduler
- Need standardized structure: alert_type, severity, title, description, affected_resource, metadata

From 08-RESEARCH.md:
- Existing Faculty Scheduler alerts pattern at faculty_scheduler.alerts
- Configuration stored in n8n_brain.preferences

## Tasks

<task id="1">
Create migration `supabase/migrations/20260124_alert_webhook_schema.sql`:
...
Add trigger for updated_at on alert_config:
...
# Verify table exists
psql -c "SELECT * FROM action_center.alert_config;"

# Verify seed data
psql -c "SELECT alert_type, display_name, default_department FROM action_center.alert_config;"
```
</verify>
...
<must_have>
alert_config table exists with all columns: alert_type, display_name, creates_tasks, default_department, info_creates_task, accumulation_threshold, accumulation_window_hours, cooldown_after_completion_hours, dismissed_cooldown_days
</must_have>

<must_have>
Default configurations seeded for: ssl_expiry, domain_health, uptime_down, payment_failed, tier_ending, vip_non_response
</must_have>
