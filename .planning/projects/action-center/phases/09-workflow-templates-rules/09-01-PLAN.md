...
phase: 9
plan: 1
title: Core Utilities - Condition Evaluation, Variable Substitution, Due Date Calculation
wave: 1
depends_on: []
files_modified:
  - dashboard/src/lib/action-center/template-utils.ts
autonomous: true
```

## Objective

Create the core utility functions that both workflow templates and task rules depend on:
1. Condition evaluation (JSON path checks with operators)
2. Variable substitution (replace ${var} in strings)
3. Due date calculation (reference date + offset)

## Context

From 09-CONTEXT.md decisions:
- Conditions use JSON path checks: `{ "field": "payload.program_type", "operator": "equals", "value": "in_person" }`
- Basic operators: equals, not_equals, in, not_in, exists, not_exists
- Multiple conditions combine with AND logic
- Variable substitution uses ${variable_name} syntax
- Due dates use days offset from reference: `{ "reference": "payload.program_date", "offset_days": -7 }`
- Calendar days by default

## Tasks

<task id="1">
Create `dashboard/src/lib/action-center/template-utils.ts` with condition evaluation:
...
Add variable substitution to `template-utils.ts`:
...
Add due date calculation to `template-utils.ts`:
...
Add dedupe key generation to `template-utils.ts`:
...
Add exports and type guards to `template-utils.ts`:
...
```bash
# Check file exists
ls -la dashboard/src/lib/action-center/template-utils.ts

# Check TypeScript compiles
cd dashboard && npx tsc --noEmit src/lib/action-center/template-utils.ts

# Check exports
grep -E "^export (function|interface|type)" dashboard/src/lib/action-center/template-utils.ts
```
</verify>
...
<must_have>
evaluateConditions() supports all 6 operators: equals, not_equals, in, not_in, exists, not_exists
</must_have>

<must_have>
substituteVariables() replaces ${varName} placeholders using variable mapping and payload
</must_have>

<must_have>
calculateDueDate() parses reference date from payload path and adds offset_days
</must_have>

<must_have>
generateDedupeKey() creates unique keys from templates with placeholder substitution
</must_have>
