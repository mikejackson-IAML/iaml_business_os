# Plan 09-03: Event Webhook Endpoint
...
phase: 9
plan: 3
title: Event Webhook Endpoint
wave: 2
depends_on: [09-01, 09-02]
files_modified:
  - dashboard/src/app/api/action-center/events/route.ts
autonomous: true
```

## Objective

Create the event webhook endpoint that receives events from external systems (n8n, other services) and routes them to matching workflow templates and task rules.

## Context

From 09-CONTEXT.md:
- Events received via POST /api/action-center/events with {event_type, entity_id, payload}
- One event can trigger multiple templates — all matching fire
- Exact match only for event types
- Force flag bypasses deduplication

## Tasks

<task id="1">
Create `dashboard/src/app/api/action-center/events/route.ts`:
...
Add GET endpoint for testing/debugging (returns recently processed events):
...
```bash
# Check file exists
ls -la dashboard/src/app/api/action-center/events/route.ts

# Check TypeScript compiles
cd dashboard && npx tsc --noEmit src/app/api/action-center/events/route.ts

# Check route exports POST and GET
grep -E "^export async function (POST|GET)" dashboard/src/app/api/action-center/events/route.ts
```
</verify>
...
<must_have>
POST /api/action-center/events accepts {event_type, entity_id, payload, force?}
</must_have>

<must_have>
Endpoint queries workflow_templates and task_rules by trigger_event
</must_have>

<must_have>
Conditions are evaluated using evaluateConditions() from template-utils
</must_have>

<must_have>
Response includes counts of matched, executed, skipped, and errors
</must_have>
