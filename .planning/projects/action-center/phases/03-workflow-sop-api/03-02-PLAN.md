...
# Plan 03-02: SOP Types and Validation

## Objective

Create TypeScript type definitions and validation utilities for the SOP Template API. These follow the same patterns established in the Task API.

## Context

From 03-RESEARCH.md:
- SOP Template entity has: id, name, description, category, department, steps (JSONB array), version, is_active, times_used, last_used_at, variables (JSONB), audit columns
- Step object structure: {order, title, description, estimated_minutes, links[], notes}
- Steps are stored as JSONB array, full replacement on update

From 03-CONTEXT.md:
- Allow empty SOPs for work-in-progress
- Version auto-increments on save (not exposed to API client)

## Tasks

<task id="1">
Create `dashboard/src/lib/api/sop-types.ts` with type definitions:
...
Create `dashboard/src/lib/api/sop-validation.ts` with validation helpers:
...
```bash
# Check files exist
ls -la dashboard/src/lib/api/sop-types.ts
ls -la dashboard/src/lib/api/sop-validation.ts

# Check TypeScript compilation
cd dashboard && npx tsc --noEmit src/lib/api/sop-types.ts src/lib/api/sop-validation.ts

# Verify exports
grep -E "^export" dashboard/src/lib/api/sop-types.ts | head -15
grep -E "^export" dashboard/src/lib/api/sop-validation.ts | head -10
```
</verify>
...
<must_have>
TypeScript types exist for: SOPStep, SOPTemplate, SOPTemplateExtended, SOPListFilters, CreateSOPRequest, UpdateSOPRequest
</must_have>

<must_have>
Validation functions exist for: validateCreateSOP, validateUpdateSOP
</must_have>

<must_have>
SOPStep validation covers: order (required positive int), title (required), description, estimated_minutes, links[], notes
</must_have>

<must_have>
Steps array validation checks for unique order numbers
</must_have>
